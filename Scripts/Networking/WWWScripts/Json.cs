...
Usage Users should not have to modify the JSONObject class themselves, and must follow the very simple proceedures outlined below: 
...
Features Decode JSON-formatted strings into a usable data structure 
...
string encodedString = j.print();NEW! The constructor, Add, and AddField functions now support a nested delegate structure. This is useful if you need to create a nested JSONObject in a single line. For example: 
...
});Not So New! (O(n)) Random access!I've added a string and int [] index to the class, so you can now retrieve data as such (from above): 
JSONObject arr = obj["field3"];
Debug.log(arr[2].n);		//Should ouptut "3"

The CodeThe code is now available on github. 
Change Logv1.4Big update! 
...
Earlier VersionsI'll fill these in later... 
}
