...
2.1 Credits 
3 Example Usage 
4 UnityScript - FastObjImporter.cs 
5 Back to general scripts 

AuthorMarc Kusters (Nighteyes) 
DescriptionThis is a basic fast ObjImporter specifically made for importing Blender obj files at runtime, it loads vertices, faces, normals and the primary uv map. 
I created this script because the current ObjImporter on this wiki was too slow to use during runtime. This importer is about 125 times faster, on a i5-4670k it imports an uv-sphere with 13k (Blender) vertices in 0.18s. 
Feel free to use this script in your projects (however, don't solely sell this in the asset store), but please give the appropriate credits. 
Note 1: This script does not fix rotation, so when exporting a file with Blender set Z = Forward, Y = Up. 
Note 2 : This script does not optimize vertex count, also it does not check if edges are smooth or hard. 
CreditsFaster int parser from [1] 
Faster float parser from [2] 

Example UsageMesh myMesh = FastObjImporter.Instance.ImportFile("path_to_obj_file.obj");UnityScript - FastObjImporter.cs/* FastObjImporter.cs
