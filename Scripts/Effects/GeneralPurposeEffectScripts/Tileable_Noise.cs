...
Description Ported Java Simplex Noise source code to C# from toxiclibs. Implemented tileable noise by mapping 4d noise into 2d plane as described in GameDev.net forum 
The method itself: 
 
SeamlessNoise( float x, float y, float dx, float dy, float xyOffset );where: x, y are normalized coordinates (in [0..1] space).
...
noise = (noise + 1.0f) * .5f;Usage example  
