...
namespace UnifyGithub.Editor.EditorScripts
{
    ...
    Setup This is an editor script, so you have to place it in an Editor folder (any subfolder named "Editor"). The filename has to be "CurveEditorTools.cs". 
    
    
    Usage To use this tool you have to open the window by clicking on "CurveEditorTools" in the "Tools" menu. You have to keep the window open while you edit an animation curve. The window will grab the last edited curve automatically which can be named and added to the preset list. You can enable / disable a preset by clicking on the "On" button. 
    When the "Overwrite Preset array" button is active (green), the Editorwindow will inject the new preset list into the AnimationCurve editor as soon as one is opened. 
    
    
    Code (CurveEditorTools.cs) // CurveEditorTools.cs
    /******************** 
     * 
    ...
    public class CurveEditorWindowWrapper
    {
        ...
        static CurveEditorWindowWrapper()
        {
            ...
            {
                if (T.Name == "CurveEditorWindow")
                {
                    m_CurveEditorWindowType = T;
                }
            }
            if (m_CurveEditorWindowType == null)
                throw new Exception("Can't get CurveEditorWindow type. Maybe it has been removed in a newer version of Unity");
     
            m_SharedInstanceField  = m_CurveEditorWindowType.GetField("s_SharedCurveEditor", BindingFlags.NonPublic | BindingFlags.Static);
            if (m_SharedInstanceField == null)
                throw new Exception("Can't get static var 's_SharedCurveEditor' of CurveEditorWindow. Maybe it has been removed in a newer version of Unity");
            m_CurveField = m_CurveEditorWindowType.GetField("m_Curve", BindingFlags.NonPublic | BindingFlags.Instance);
            if (m_CurveField == null)
                throw new Exception("Can't get var 'm_Curve' of CurveEditorWindow. Maybe it has been removed in a newer version of Unity");
            m_PresetsField = m_CurveEditorWindowType.GetField("m_Presets", BindingFlags.NonPublic | BindingFlags.Instance);
            if (m_PresetsField == null)
                throw new Exception("Can't get var 'm_PresetsField' of CurveEditorWindow. Maybe it has been removed in a newer version of Unity");
        }
